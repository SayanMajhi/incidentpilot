...
| `Claude outputs/` | Notes or generated change summaries from prior assistant work. |
...
| `safety/` | Top-level cache-only folder in this workspace; real safety code is in `backend/safety/`. |
| `scripts/` | Helper scripts for Kubernetes, Qwen connection checks, and demo automation. |
| `simulator/` | Top-level cache-only folder in this workspace; real simulator code is in `backend/simulator/`. |
| `tests/` | Backend test suite plus Kubernetes fakes and integration tests. |
| `tools/` | Top-level cache-only folder in this workspace; real tool code is in `backend/tools/`. |
| `verification/` | Top-level cache-only folder in this workspace; real verification code is in `backend/verification/`. |
...
| `frontend/tsconfig.tsbuildinfo` | Generated TypeScript build cache; safe to ignore. |
...
| `frontend/dist/` | Generated production frontend build output. |
| `frontend/node_modules/` | Installed npm dependencies; generated by `npm ci`. |
...
| Path | What it does |
| --- | --- |
| `Claude outputs/CHANGES.md` | A prior generated change summary; useful as history, not app runtime code. |
