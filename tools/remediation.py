...
simulated production service in `simulator/service.py`.
...
Design principle - actions report on themselves, not on the incident:
    ...
    `tools/diagnostics.py`) can answer, by re-checking health/metrics
...
# ---------------------------------------------------------------------------
# The simulator (simulator/service.py) doesn't model replica count, so
...
def restart_service() -> Dict[str, Union[str, bool]]:
    ...
    return {
        ...
        "message": (
            ...
            "This reports only that the restart action itself completed - "
            "it says nothing about the underlying incident. Verify current "
            "status separately, e.g. with diagnostics.check_health()."
        ),
    }
